...
data = list(zip(users_agents_values, expected_values))


@pytest.mark.parametrize('user_agent_val, expected', data)
def test_user_agent(user_agent_val, expected):
    response = requests.get(
        "https://playground.learnqa.ru/ajax/api/user_agent_check",
        headers={"User-Agent": user_agent_val})

    # assert "User-Agent" in response.json(), "The 'User-Agent' is not in the response"
    assert expected in response.json(), "не совпадает с ответом"
